...
public class UserController {
	...
	@ResponseBody
	public com.sys.entity.LoginUser getUser() {
		com.sys.entity.LoginUser loginUser = (com.sys.entity.LoginUser) SecurityUtils.getCurrentUserAuthentication().getPrincipal();
		return loginUser;
	}
	...
	public PageResult getUsers(PageTableRequest request) {
		request.countOffset();
		return userService.getAllUsersByPage(request.getOffset(),request.getRows());
	}
	...
	@ResponseBody
	@PreAuthorize("hasAuthority('sys:user:edit')")
	@ApiOperation(value = "重置用户密码", notes = "重置成功，密码为：123456")
	public Results<SysUser> resetUser(@RequestBody UserDto userDto){
		userService.resetUser(userDto.getUserIds());
		return Results.success("重置成功，密码为：123456");
	}
	...
	@ApiOperation("用户注册")
	@ResponseBody
	public Results doRegister(UserDto userDto){
		SysUser sysUser = null;
		sysUser = userService.getUser(userDto.getUsername());
		if(sysUser != null && !(sysUser.getId().equals(userDto.getId()))){
			return Results.failure(ResponseCode.USERNAME_REPEAT.getCode(),ResponseCode.USERNAME_REPEAT.getMessage());
		...
		userDto.setPassword(new BCryptPasswordEncoder().encode(userDto.getPassword()));
		//userDto.setPassword(MD5.crypt(userDto.getPassword()));
		return userService.save(userDto,5);
	}
